...
var RANK_ANALYSIS_CONFIG = {
  ...
  // ── 시트 구조 ───────────────────────────────────────────────────────────
  // 실제 B시트는 1~5행이 봇 안내문(사용법·버튼)이고 진짜 헤더는 6행이다.
  HEADER_ROW: 6,
  ...
  // 실제 B시트 4행 안내문에 "확인 불가가 뜨는 경우 - 상품이 1000위 밖에 있을 경우"라고
  // 적혀 있고, 실제 데이터에도 999가 같은 상품에 수십 번 연속으로 반복된다.
  // 진짜 999위라기보다 "1000위 밖(측정 상한)"을 뜻하는 것으로 판단해 기본으로 켜 둔다.
  // 999가 실제 정상 순위인 경우가 있었다면 이 값을 빈 배열 [] 로 바꾸면 된다.
  RANK_CAP_VALUES: [999],
